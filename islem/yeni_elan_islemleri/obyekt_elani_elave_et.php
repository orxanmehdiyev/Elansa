...
$emlaksenedi=$db->prepare("SELECT * FROM obyekt where obyekt_id=:obyekt_id");
$emlaksenedi->execute(array(
	'obyekt_id'=>$obyekt_id));
$emlaksenedisay=$emlaksenedi->rowCount();
if ($emlaksenedisay==1) {
	$emlaksenedicek=$emlaksenedi->fetch(PDO::FETCH_ASSOC);
	$elan_adi=$emlaksenedicek["obyekt_adi"];

}else{
	header("Location:../yenielan.php?durum=obyekttipixeta");
	exit;
}
