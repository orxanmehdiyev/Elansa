...
<section>
		...
		<div id="seyfebaslikkapsayici">
			...
			if(isset($durum)){
				...
				<?php  }
					...
					<span >Mənzil Təhcizat Bölməsi</span>
				<?php }
			} else{ ?>
				<span >Mənzil Təhcizat Bölməsi</span>
			<?php  }
			...
			if(isset($_SESSION['menziltechizatbolmesisiralama'])){
				$Siralama=$_SESSION['menziltechizatbolmesisiralama'];			
				$link='MenzilTechizatBolmesi?';
			}else{
				
				$Siralama="MenziTechizatBolmesiID ASC";
				$link='MenzilTechizatBolmesi?';
			}
	...
	<div id="ListelemeAlaniKapsayicisi">
		<div class="SayfaIciBaslikAlanlariKapsayicisi">
			<a href="MenzilTechizatBolmesi">Mənzil Təhcizat Bölməsi</a>
		...
		if(isset($_REQUEST['axtar'])){
			...
			$sorsay = $db->prepare("SELECT * FROM menzil_techizat_bolmesi WHERE MenziTechizatBolmesiAd LIKE ?");
			...
			$SeyfeBaslangicKayitSayi    = ($GelenSeyfeleme*$menzil_techizat_bolmesi_listeleme_limiti)-$menzil_techizat_bolmesi_listeleme_limiti;
			$BulunanSeyfeAxtarisSayisi  = ceil($say/$menzil_techizat_bolmesi_listeleme_limiti);
			$sor = $db->prepare("SELECT* FROM menzil_techizat_bolmesi  WHERE MenziTechizatBolmesiAd LIKE ? order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $menzil_techizat_bolmesi_listeleme_limiti");
			...
			if ($say>0) {
				...
				<div id="SayfalamaAlaniKapsayicisi">
					<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
						<?php echo $BulunanSeyfeAxtarisSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Mənzil Təhcizat Bölməsi Var.   <label for="cars">Göstərilən:</label>
						<select onchange="ListelemeLimiti(this.value)">
							...
							for ($i = 10; $i <= $Limitsayi; $i+=10 ) {	?>
								<option <?php if($menzil_techizat_bolmesi_listeleme_limiti==$i){echo "selected";} ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>

							<?php }	
							if (($say%10)>0) { ?>
								<option <?php if($menzil_techizat_bolmesi_listeleme_limiti==$say){echo "selected";} ?> value="<?php echo $say; ?>"><?php echo $say; ?></option>
							<?php }
					...
					if ($BulunanSeyfeAxtarisSayisi>1) {?>
						<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
							<?php
							if($GelenSeyfeleme<1){
								header("Location:MenzilTechizatBolmesi?seyfe=$BulunanSeyfeAxtarisSayisi");
							}
							if ($GelenSeyfeleme>1) {
			...
			$Say_sor = $db->prepare("SELECT * FROM menzil_techizat_bolmesi");
			$Say_sor->execute();
			...
			$SeyfeBaslangicKayitSayi   = ($GelenSeyfeleme*$menzil_techizat_bolmesi_listeleme_limiti)-$menzil_techizat_bolmesi_listeleme_limiti;
			$BulunanSayfaSayisi                 = ceil($say/$menzil_techizat_bolmesi_listeleme_limiti);
			$sor = $db->prepare("SELECT * FROM menzil_techizat_bolmesi order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $menzil_techizat_bolmesi_listeleme_limiti");
			$sor->execute();
			...
			<div id="SayfalamaAlaniKapsayicisi">
				<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
					<?php echo $BulunanSayfaSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Mənzil Təhcizat Bölməsi Var. Göstərilən
					<select onchange="ListelemeLimiti(this.value)">
						<?php 
						if ($say>100) {
							$Limitsayi=100;
				...
				if ($BulunanSayfaSayisi>1) {?>
					<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
						<?php
						if($GelenSeyfeleme<1){
							header("Location:MenzilTechizatBolmesi?seyfe=$BulunanSayfaSayisi");
						}
						if ($GelenSeyfeleme>1) {
							$SayfalamaIcinSayfaDegeriniBirGeriAl = $GelenSeyfeleme-1;
