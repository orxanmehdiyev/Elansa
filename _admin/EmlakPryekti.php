...
<section>
	<div id="seyfebaslikalani">
		<div id="seyfebaslikkapsayici">
			...
			if(isset($_SESSION['proyektnovusiralama'])){
				$Siralama=$_SESSION['proyektnovusiralama'];			
				$link='EmlakPryekti?';
			}else{
				
				$Siralama="proyekt_id ASC";
				$link='EmlakPryekti?';
			}
	...
	<div id="ListelemeAlaniKapsayicisi">
		...
		if(isset($_REQUEST['axtar'])){
			...
			$sorsay = $db->prepare("SELECT * FROM proyekt WHERE proyekt_ad LIKE ?");
			...
			$SeyfeBaslangicKayitSayi    = ($GelenSeyfeleme*$proyekt_listeleme_limiti)-$proyekt_listeleme_limiti;
			$BulunanSeyfeAxtarisSayisi  = ceil($say/$proyekt_listeleme_limiti);
			$sor = $db->prepare("SELECT* FROM proyekt  WHERE proyekt_ad LIKE ? order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $proyekt_listeleme_limiti");
			...
			if ($say>0) {
						...
						<thead>
							<th class="ListelemeAlaniIciTablosuSiraNomiresi">
								<form action="Islem/proyekt_islem.php" method="POST">
									...
									<?php if ($_SESSION['proyektnovusiralama']=="proyekt_id ASC") {
							...
							<th>
								<form action="Islem/proyekt_islem.php" method="POST">
									<input type="hidden" name="siralama"
									<?php if ($_SESSION['proyektnovusiralama']=="proyekt_ad ASC") {
									...
									<td class="KodSutunu">	
										<label class="checkbox" title="<?php if($cek['menziller_ucun_proyekt_durum']==1){echo 'Passif Et';}else{ echo 'Aktiv Et';}?>" >
											<input <?php if($cek['menziller_ucun_proyekt_durum']==1){
												...
												echo "";
											} ?>  type="checkbox" id="<?php echo 'menziller_ucun_proyekt_durum-'.$cek['proyekt_id'] ?>" onchange="DurumKontrol(this.id)" > 
											<span class="checkbox"> 
												<span></span>
											</span>
										</label>
				...
				<div id="SayfalamaAlaniKapsayicisi">
					<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
						<?php echo $BulunanSeyfeAxtarisSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Proyekt Növü Var.   <label for="cars">Göstərilən:</label>
						<select onchange="ListelemeLimiti(this.value)">
							...
							for ($i = 10; $i <= $Limitsayi; $i+=10 ) {	?>
								<option <?php if($proyekt_listeleme_limiti==$i){echo "selected";} ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>

							<?php }	
							if (($say%10)>0) { ?>
								<option <?php if($proyekt_listeleme_limiti==$say){echo "selected";} ?> value="<?php echo $say; ?>"><?php echo $say; ?></option>
							<?php }
					...
					if ($BulunanSeyfeAxtarisSayisi>1) {?>
						<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
							<?php
							if($GelenSeyfeleme<1){
								header("Location:EmlakPryekti?seyfe=$BulunanSeyfeAxtarisSayisi");
							}
							if ($GelenSeyfeleme>1) {
			...
			$Say_sor = $db->prepare("SELECT * FROM proyekt");
			$Say_sor->execute();
			...
			$SeyfeBaslangicKayitSayi   = ($GelenSeyfeleme*$proyekt_listeleme_limiti)-$proyekt_listeleme_limiti;
			$BulunanSayfaSayisi                 = ceil($say/$proyekt_listeleme_limiti);
			$sor = $db->prepare("SELECT * FROM proyekt order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $proyekt_listeleme_limiti");
			$sor->execute();
			...
			<div id="SayfalamaAlaniKapsayicisi">
				<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
					<?php echo $BulunanSayfaSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Proyekt Növü Var. Göstərilən
					<select onchange="ListelemeLimiti(this.value)">
						<?php 
						if ($say>100) {
							$Limitsayi=100;
				...
				if ($BulunanSayfaSayisi>1) {?>
					<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
						<?php
						if($GelenSeyfeleme<1){
							header("Location:EmlakPryekti?seyfe=$BulunanSayfaSayisi");
						}
						if ($GelenSeyfeleme>1) {
							$SayfalamaIcinSayfaDegeriniBirGeriAl = $GelenSeyfeleme-1;
