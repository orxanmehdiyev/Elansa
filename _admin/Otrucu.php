...
<section>
	<div id="seyfebaslikalani">
		<div id="seyfebaslikkapsayici">
			...
			if(isset($_SESSION['otrucusiralama'])){
				$Siralama=$_SESSION['otrucusiralama'];			
				$link='Otrucu?';
			}else{
				
				$Siralama="avto_otrucu_id ASC";
				$link='Otrucu?';
			}
	...
	<div id="ListelemeAlaniKapsayicisi">
		...
		if(isset($_REQUEST['axtar'])){
			...
			$sorsay = $db->prepare("SELECT * FROM avto_otrucu WHERE avto_otrucu_ad LIKE ?");
			...
			$SeyfeBaslangicKayitSayi    = ($GelenSeyfeleme*$avto_otrucu_listeleme_limiti)-$avto_otrucu_listeleme_limiti;
			$BulunanSeyfeAxtarisSayisi  = ceil($say/$avto_otrucu_listeleme_limiti);
			$sor = $db->prepare("SELECT* FROM avto_otrucu  WHERE avto_otrucu_ad LIKE ? order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $avto_otrucu_listeleme_limiti");
			...
			if ($say>0) {
									...
									<td class="KodSutunu">	
										<label class="checkbox" title="<?php if($cek['avto_otrucudurum']==1){echo 'Passif Et';}else{ echo 'Aktiv Et';}?>" >
											<input <?php if($cek['avto_otrucudurum']==1){
												...
												echo "";
											} ?>  type="checkbox" id="<?php echo 'avto_otrucudurum-'.$cek['avto_otrucu_id'] ?>" onchange="DurumKontrol(this.id)" > 
											<span class="checkbox"> 
												<span></span>
											</span>
										</label>
				...
				<div id="SayfalamaAlaniKapsayicisi">
					<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
						<?php echo $BulunanSeyfeAxtarisSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Ötrücü Var.   <label for="cars">Göstərilən:</label>
						<select onchange="ListelemeLimiti(this.value)">
							...
							for ($i = 10; $i <= $Limitsayi; $i+=10 ) {	?>
								<option <?php if($avto_otrucu_listeleme_limiti==$i){echo "selected";} ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>

							<?php }	
							if (($say%10)>0) { ?>
								<option <?php if($avto_otrucu_listeleme_limiti==$say){echo "selected";} ?> value="<?php echo $say; ?>"><?php echo $say; ?></option>
							<?php }
					...
					if ($BulunanSeyfeAxtarisSayisi>1) {?>
						<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
							<?php
							if($GelenSeyfeleme<1){
								header("Location:Otrucu?seyfe=$BulunanSeyfeAxtarisSayisi");
							}
							if ($GelenSeyfeleme>1) {
			...
			$Say_sor = $db->prepare("SELECT * FROM avto_otrucu");
			$Say_sor->execute();
			...
			$SeyfeBaslangicKayitSayi   = ($GelenSeyfeleme*$avto_otrucu_listeleme_limiti)-$avto_otrucu_listeleme_limiti;
			$BulunanSayfaSayisi                 = ceil($say/$avto_otrucu_listeleme_limiti);
			$sor = $db->prepare("SELECT * FROM avto_otrucu order by $Siralama LIMIT $SeyfeBaslangicKayitSayi, $avto_otrucu_listeleme_limiti");
			$sor->execute();
			...
			<div id="SayfalamaAlaniKapsayicisi">
				<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
					<?php echo $BulunanSayfaSayisi; ?> səyfədə, <?php echo $say; ?> Qeydiyatlı Ötrücü Var. Göstərilən
					<select onchange="ListelemeLimiti(this.value)">
						<?php 
						if ($say>100) {
							$Limitsayi=100;
				...
				if ($BulunanSayfaSayisi>1) {?>
					<div class="SayfalamaAlaniIciNumaralandirmaAlaniKapsayicisi">
						<?php
						if($GelenSeyfeleme<1){
							header("Location:Otrucu?seyfe=$BulunanSayfaSayisi");
						}
						if ($GelenSeyfeleme>1) {
							$SayfalamaIcinSayfaDegeriniBirGeriAl = $GelenSeyfeleme-1;
