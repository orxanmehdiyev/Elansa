...
class timthumb {
	...
	public function __construct(){
		...
		$this->src = "/timthumb_ornek/".$this->param('folder')."/".$this->param('src');
	...
	protected function processImageAndWriteToCache($localImage){
		...
		// get standard input properties	$image_setting = explode("/",IC($_GET["setting"],1));
		...
		/*
			...
			-----------------------
			$new_width =  (int) abs($prop_parameter[0]);
			$new_height = (int) abs($prop_parameter[1]);
			$zoom_crop = (int) $this->param('zc', DEFAULT_ZC);
			$quality =  (int) abs($prop_parameter[2]);
			$align = $this->cropTop ? 't' : $this->param('a', 'c');
			$filters = $this->param('f', DEFAULT_F);
			$sharpen = (bool) $this->param('s', DEFAULT_S);
			$canvas_color = $this->param('cc', DEFAULT_CC);
		...
		*/
		...
		$prop_parameter = explode('-',$this->param('prop'));
		
		$new_width =  (int) abs($prop_parameter[0]?$prop_parameter[0]:0);
		$new_height = (int) abs($prop_parameter[1]?$prop_parameter[1]:0);
		$quality = (int) abs ($prop_parameter[2]?$prop_parameter[2]:DEFAULT_Q);
		$filters = $prop_parameter[3]?$prop_parameter[3]:DEFAULT_F;
		$zoom_crop = (int) $prop_parameter[4]?$prop_parameter[4]:DEFAULT_ZC;
		$sharpen = (bool) $prop_parameter[5]?$prop_parameter[5]:DEFAULT_S;
		$canvas_color = $prop_parameter[6]?$prop_parameter[6]:DEFAULT_CC;
		$canvas_trans = (bool) $prop_parameter[7]?$prop_parameter[7]:'1';
		$align = $this->cropTop ? 't' : $prop_parameter[8]?$prop_parameter[8]: $prop_parameter[9];
